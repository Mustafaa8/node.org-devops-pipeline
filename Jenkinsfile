pipeline {
    ...
    stages {
        ...
        stage('Docker Image Scanning') {
            steps {
                sh '''
                docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v ./:/output -w /output aquasec/trivy --timout 20m image ${IMAGE_NAME}:${IMAGE_TAG}
                '''
            }
        }
        stage('Docker Composing') {
            steps {
                sh 'docker compose -f ./compose/compose.yml up -d'
            }
        }
    }
}